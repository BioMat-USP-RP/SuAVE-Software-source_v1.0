...
program thickness
  ...
  real :: la, lb, lc, start, finish, minv, maxv, del, gridx, gridy
  ...
  if (.not.outer)then
     ...
     if(ierr>0) then
        ...
        write(*, *)
        write(*, *) 'Problem by reading ', ind, aux
        write(*, *)
        stop
     ...
     if(ierr>0) then
        ...
        write(*, *)
        write(*, *) 'Problem by reading ', ind2, aux
        write(*, *)
        stop
...
2 format (a10)
  ...
  if (ex) then
     ...
     write(*, *)
     write(*, *) 'Previous thick.xvg file is now backed up to thick_p.xvg !'
  ...
  if (ex) then
     ...
     write(*, *)
     write(*, *) 'Previous volume.xvg file is now backed up to volume_p.xvg !'
...
12   format(a6, i5.1, a5, a5, a1, i4.1, a4, 3f8.3)
           ...
           if ((frame>fr_in-1).and.(frame<fr_end+1).and.(eval_skip)) then
              ...
              do i=2, n_grid+1
                 ...
                 do j=2, n_grid+1
                    ...
                    v3%x = - v3%x/v3%z
                    v3%y = - v3%y/v3%z
                    v3%z = - v3%x*(grid(i-1,j-1)%x+dx/2) - v3%y*(grid(i-1,j-1)%y+dy/2) + la
                    
                    s_v = s_v + v3%x*dx*dy*(dx+2*grid(i-1,j-1)%x)/2 + v3%y*dy*dx*(dy+2*grid(i-1,j-1)%y)/2 + v3%z*dx*dy
                    
                    ! superficie de baixo =====================================================
                    v1%x = grid2(i,j)%x-grid2(i-1,j-1)%x
                    v1%y = grid2(i,j)%y-grid2(i-1,j-1)%y
                    v1%z = grid2(i,j)%z-grid2(i-1,j-1)%z
                    
                    v2%x = grid2(i-1,j)%x-grid2(i,j-1)%x
                    v2%y = grid2(i-1,j)%y-grid2(i,j-1)%y
                    v2%z = grid2(i-1,j)%z-grid2(i,j-1)%z
                    ...
                    v3%x = - v3%x/v3%z
                    v3%y = - v3%y/v3%z
                    v3%z = - v3%x*(grid2(i-1,j-1)%x+dx/2) - v3%y*(grid2(i-1,j-1)%y+dy/2) + lb
                    
                    s_v = s_v - v3%x*dx*dy*(dx+2*grid2(i-1,j-1)%x)/2 - v3%y*dy*dx*(dy+2*grid2(i-1,j-1)%y)/2 - v3%z*dx*dy
                    
                    r_xpm(i-1,j-1) = r_xpm(i-1, j-1) + abs(la-lb)
                    
                 end do
                 
              end do
              
              write(4, *) frame, abs(s_v)/(1000*n_lipid*2)
              ...
              do i=1, n_grid+1
                 ...
                 do j=1, n_grid+1
                    ...
                    aux2 = abs((grid(i,j)%z-grid2(i,j)%z))/10
  ...
  if (ex) then
     ...
     write(*, *)
     write(*, *) 'Previous thick.xpm file is now backed up to thick_p.xpm !'
